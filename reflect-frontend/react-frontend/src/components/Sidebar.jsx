...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <button
        onClick={onToggle}
        className={`lg:hidden fixed top-4 left-4 z-50 p-2 rounded-lg bg-zinc-900 hover:bg-zinc-800 transition-colors ${
      ...
      <aside
        className={`
          ...
          ${timeOfDay === 'morning' ? 'border-blue-400/20' : 'border-amber-400/20'}
        `}
      >
        {/* Decorative glow */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className={`absolute -top-24 -right-24 w-96 h-96 rounded-full blur-3xl ${timeOfDay === 'morning' ? 'bg-blue-500/5' : 'bg-amber-500/5'}`}></div>
          <div className={`absolute -bottom-24 -left-24 w-96 h-96 rounded-full blur-3xl ${timeOfDay === 'morning' ? 'bg-cyan-500/5' : 'bg-orange-500/5'}`}></div>
        </div>

        <div className="relative h-full flex flex-col">
          {/* Header */}
          <div className={`p-6 border-b ${timeOfDay === 'morning' ? 'border-blue-400/20' : 'border-amber-400/20'}`}>
            <div className="flex items-center gap-3">
              ...
              {isOpen && (
                <div>
                  <h1 className={`text-xl font-bold tracking-tight ${timeOfDay === 'morning' ? 'text-blue-400' : 'text-amber-400'}`} style={{ fontFamily: "'Playfair Display', serif" }}>
                    Reflect
                  </h1>
          ...
          {isOpen && (
            <div className="px-4 py-4">
              <div className={`rounded-xl p-3 border ${
                timeOfDay === 'morning'
                  ? 'bg-gradient-to-r from-blue-500/10 to-sky-500/10 border-blue-400/20'
                  : 'bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-amber-400/20'
              }`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-zinc-400 uppercase tracking-wider">Current Streak</p>
                    <p className={`text-2xl font-bold ${timeOfDay === 'morning' ? 'text-blue-400' : 'text-amber-400'}`}>{currentStreak} days</p>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <li key={item.id}>
                    <button
                      ...
                      className={`
                        ...
                        ${isActive
                          ? timeOfDay === 'morning'
                            ? 'bg-gradient-to-r from-blue-400/20 to-sky-400/10 text-blue-400 shadow-lg shadow-blue-500/10'
                            : 'bg-gradient-to-r from-amber-400/20 to-orange-400/10 text-amber-400 shadow-lg shadow-amber-500/10'
                          : item.highlight
                            ? timeOfDay === 'morning'
                              ? 'text-blue-300 bg-blue-500/5 hover:bg-blue-500/10'
          ...
          {/* Footer - User */}
          <div className={`p-4 border-t ${timeOfDay === 'morning' ? 'border-blue-400/20' : 'border-amber-400/20'}`}>
            <div className={`flex items-center gap-3 p-3 rounded-xl bg-zinc-800/50 hover:bg-zinc-800 transition-all cursor-pointer ${!isOpen && 'justify-center'}`}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-zinc-900 font-semibold shadow-lg ${
                timeOfDay === 'morning'
              ...
              {isOpen && (
                <div className="flex-1">
                  <p className="text-sm font-medium text-zinc-200">Rahul Iyer</p>
